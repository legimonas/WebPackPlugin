...
import by.bsu.webpack.explorer.ui.dialogs.NewEntityDialog
...
class NewEntityAction : AnAction() {
  override fun actionPerformed(e: AnActionEvent) {
    ...
    val dialog = NewEntityDialog(e.project, webPackProjectConfigOpt.get())
    if (dialog.showAndGet()) {
      dataProvider.add(dialog.state.toEntityConfig(webPackProjectConfigOpt.get()))
    }
  }
  ...
  override fun update(e: AnActionEvent) {
    ...
    if (selectedNode.nodeType != NodeType.ENTITIES_FOLDER && selectedNode.nodeType !== NodeType.PROJECT) {
      e.presentation.isEnabledAndVisible = false
      return
    }
